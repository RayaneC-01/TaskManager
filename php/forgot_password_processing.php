...
if (isset ($_POST['token'], $_POST['password'], $_POST['confirm_password'])) {
    ...
    if ($password === $confirm_password) {
        ...
        // Remplacez 'users' par le nom de votre table d'utilisateurs
        require 'connection_database.php'; // Assurez-vous d'inclure le fichier de connexion à la base de données
        $update_stmt = $conn->prepare("UPDATE users SET password = :password WHERE reset_token = :token");
        $update_stmt->bindParam(':password', $hashed_password);
        $update_stmt->bindParam(':token', $token);
        $update_stmt->execute();

        // Récupérer l'e-mail de l'utilisateur associé au token de réinitialisation
        $select_email_stmt = $conn->prepare("SELECT email FROM users WHERE reset_token = :token");
        $select_email_stmt->bindParam(':token', $token);
        $select_email_stmt->execute();
        $user_email = $select_email_stmt->fetchColumn();

        // Envoyer l'e-mail de confirmation
        $subject = "Confirmation de réinitialisation de mot de passe";
        $message = "Votre mot de passe a été réinitialisé avec succès.";
        $headers = "From: " . $user_email; // Utilisez l'e-mail de l'utilisateur comme expéditeur
